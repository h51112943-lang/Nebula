...
def _ast_deep_scan(content, file_path):
    ...
    def _resolve_attr_chain(node):
        ...
        if isinstance(cur, ast.Name):
            base = alias_map.get(cur.id, cur.id)
            parts.append(base)
        parts.reverse()
        if len(parts) >= 2:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            # --- tehlikeli modül.fonksiyon zincirleri (alias'lar dahil) ---
            if isinstance(node.func, ast.Attribute):
                ...
                if chain:
                    ...
                    if chain in _DANGEROUS_ATTR_CHAINS_CRITICAL and dynamic_arg:
                        # Sadece os.system/subprocess.* çağrısına GERÇEKTEN dinamik/hesaplanmış
                        # bir komut veriliyorsa kritik; sabit string komutla çağırmak
                        # (ör. os.system("clear")) çoğu meşru botta zararsızca kullanılır.
                        findings.append((
                            SEV_CRITICAL,
                            f"'{mod}.{fn}()' çağrısına dinamik/hesaplanmış komut veriliyor "
                            f"(satır {getattr(node, 'lineno', '?')})",
                            True,
                        ))
                    elif chain in _DANGEROUS_ATTR_CHAINS_CRITICAL:
                        findings.append((
                            SEV_MEDIUM,
                            f"'{mod}.{fn}()' çağrısı tespit edildi (satır {getattr(node, 'lineno', '?')})",
                            False,
                        ))
                    elif chain in _DANGEROUS_ATTR_CHAINS_HIGH:
                        findings.append((
                            SEV_HIGH if dynamic_arg else SEV_MEDIUM,
                            f"'{mod}.{fn}()' çağrısı tespit edildi (satır {getattr(node, 'lineno', '?')})",
                            False,
                        ))
...
def activate_package(uid, pkg_id):
    ...
    if minutes > 0:
        ...
        info = f"⏳ {format_duration(minutes)} boyunca aktif. (Bitiş: {until})"
    else:
        until = None
